...
import { db } from "@/db";
import { trip, customer, admin } from "@/db/schema";
import { eq, desc, ilike, gte, lte, and, or, sql } from "drizzle-orm";
import Navbar from "@/components/layout/Navbar";
import Link from "next/link";

export default async function AdminTripsPage(props: { searchParams?: Promise<{ q?: string; from?: string; to?: string }> }) {
  ...
  if (from) {
    filters.push(gte(trip.date, from));
  }

  if (to) {
    filters.push(lte(trip.date, to));
  }
