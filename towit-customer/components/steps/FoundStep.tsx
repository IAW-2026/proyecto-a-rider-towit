...
export default function FoundStep({ driverName, driverRating, craneTypeLabel, eta }: FoundStepProps) {
  return (
    <div className="flex flex-col items-center text-center space-y-6 py-8">
      <div className="w-20 h-20 flex items-center justify-center mx-auto">
        <div className="mb-5 inline-flex h-14 w-14 items-center justify-center rounded-xl">
          <img src="/images/towicon/tow.svg" alt="Tow It" width="56" height="56" />
        </div>
      </div>
      ...
      <div className="bg-card p-5 rounded-xl border border-border shadow-sm text-left w-full max-w-sm">
        ...
        <div className="flex justify-between items-center">
          <p className="text-lg font-bold text-foreground">{driverName || "Conductor asignado"} • Grúa {craneTypeLabel}</p>
          <div className="bg-muted px-3 py-1 rounded-full">
            <FontAwesomeIcon icon={faStar} className="text-xs text-brand-yellow mr-1" />
            <span className="font-bold text-foreground text-sm">{driverRating > 0 ? driverRating : "—"}</span>
          </div>
        </div>
